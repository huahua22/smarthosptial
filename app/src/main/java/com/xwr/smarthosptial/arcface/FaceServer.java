...
import com.xwr.smarthosptial.arcface.common.Const;
import com.xwr.smarthosptial.arcface.model.CompareResult;
import com.xwr.smarthosptial.arcface.model.FaceRegisterInfo;
...
public class FaceServer {
    ...
    public boolean init(Context context) {
        synchronized (this) {
            if (faceEngine == null && context != null) {
                ...
                int activeCode = faceEngine.activeOnline(context, Const.APP_ID, Const.SDK_KEY);
                if (activeCode != ErrorInfo.MOK && activeCode != ErrorInfo.MERR_ASF_ALREADY_ACTIVATED) {
                    return false;
                }
                int engineCode = faceEngine.init(context, DetectMode.ASF_DETECT_MODE_VIDEO, DetectFaceOrientPriority.ASF_OP_0_ONLY, 16, 10, FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT);
    ...
    public File getFeatureDir(Context context){
        if (ROOT_PATH == null) {
            ROOT_PATH = context.getFilesDir().getAbsolutePath();
        }
        //特征存储的文件夹
    ...
    public File getImgDir(Context context){
        if (ROOT_PATH == null) {
            ROOT_PATH = context.getFilesDir().getAbsolutePath();
        }
        //图片存储的文件夹
